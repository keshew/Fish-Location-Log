...
extension LoadingView {
    ...
    func req() {
        ...
        conversionData["bundle_id"] = "com.app.fishlocationlog"
        conversionData["os"] = "iOS"
        conversionData["store_id"] = "6757950087"
        conversionData["locale"] = Locale.current.identifier
        conversionData["firebase_project_id"] = "496970222528"

        do {
            let jsonData = try JSONSerialization.data(withJSONObject: conversionData, options: [])
                    let url = URL(string: "https://placelocationlog.com/config.php")!
